...
import { LoadUser, LoginUser, RegisterUser } from '@/app/apicalls'
...
const Page = () => {
  ...
  const HandleLoginUser = async (e: any) => {
    e.preventDefault();
    await RegisterUser(UserName, UserEmail, UserPassword, dispatch);
    setUserName(""); 
    setUserEmail("");
    setUserPassword("");    
  }
   return (
      <div className='h-screen w-screen flex items-center justify-center bg-no-repeat bg-cover' style={{background:`url(${`https://imgs.search.brave.com/OswiSy3iBPRIV7a28QCe1Cy9fJK0oyA6EkfwCxzFBEA/rs:fit:860:0:0/g:ce/aHR0cHM6Ly93YWxs/cGFwZXJjYXZlLmNv/bS93cC93cDcwOTQ1/NTkuanBn`})`,backgroundSize:"cover",backgroundRepeat:"no-repeat"}}>
        ...
        <form className='h-130 w-170 flex flex-col items-center gap-4 md:w-120' method='POST' onSubmit={HandleLoginUser}>
           <Input type="text" placeholder="Name" onChange={e => setUserName(e.target.value)} value={UserName} required/>
           <Input type="email" placeholder="Email" onChange={e => setUserEmail(e.target.value)} value={UserEmail} required/>
           <Input type="password" placeholder="Password" onChange={e => setUserPassword(e.target.value)} value={UserPassword} required/>
           <Button type='submit' className='px-4 py-2'>{Loading?<Loader/>:"Register" }</Button>
              </form>
              <div className='flex items-center gap-1 text-sm'>Already Have An Account <Button variant={"link"} onClick={()=>router.push("/client/login")}>Login Here</Button> </div>
